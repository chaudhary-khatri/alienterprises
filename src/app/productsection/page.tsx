...
const ProductSection = () => {
  ...
  return (
    <>
      <Head>
        <title>Our Products – Ali Enterprises</title>
        <meta
          name="description"
          content="Explore our diverse range of quality products at Ali Enterprises. Find the best machinery and industrial solutions for your business."
        />
        {/* Open Graph Meta Tags */}
        <meta property="og:title" content="Our Products – Ali Enterprises" />
        <meta
          property="og:description"
          content="Explore our diverse range of quality products at Ali Enterprises. Find the best machinery and industrial solutions for your business."
        />
        <meta property="og:url" content="https://alienterprises.in/products" />
        <meta property="og:type" content="website" />
        {/* Twitter Card Meta Tags */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Our Products – Ali Enterprises" />
        <meta
                      ...
                      >
                        ...
                        <div className="absolute inset-0 z-10 flex flex-col justify-between">
                          <div className="text-left">
                            <div className="bg-gradient-to-r from-teal-600 to-teal-800 inline-block rounded-lg shadow-lg">
                              <h3 className="text-3xl p-4 font-semibold text-white">
                                {product.name}
                              </h3>
                            </div>
                          </div>
                          {product.description && (
                            <p className="text-sm border border-gray-400 p-2 text-gray-900 bg-gray-300 bg-opacity-90">
                              {product.description}
                            </p>
                          )}
                        </div>
                ...
                <div className="space-y-6">
                  {products.map((product, index) => (
                    <button
                      key={product.name}
                      onClick={() => handleManualChange(index)}
                      className={`w-full text-left px-6 py-4 rounded-lg font-medium transition-all duration-300 ${currentProduct === index
          ...
          <div className="lg:hidden w-full mt-8">
            <div className="w-full flex flex-col bg-gray-300 rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="flex justify-center items-center space-x-6 px-4 py-6 bg-teal-500 rounded-b-xl">
                <button
                  className="flex items-center bg-teal-700 text-white p-4 rounded-2xl shadow-xl transition-all hover:bg-gray-700"
                  onClick={() =>
                    handleManualChange((currentProduct - 1 + products.length) % products.length)
                  }
